...
class CmdVelPublisher(Node):
    ...
    def publish_cmd_vel(self):
        twist_msg = Twist()
        twist_msg.linear.x = 0.1  # Adjust the linear velocity as needed
        ...
        time.sleep(5)
        twist_msg.linear.x = 0.0  # Adjust the linear velocity as needed
        twist_msg.angular.z = 0.00  # Adjust the angular velocity as needed
        self.publisher_.publish(twist_msg)
        self.get_logger().info('Publishing cmd_vel: linear=%.2f, angular=%.2f' % (twist_msg.linear.x, twist_msg.angular.z))
        time.sleep(5)
